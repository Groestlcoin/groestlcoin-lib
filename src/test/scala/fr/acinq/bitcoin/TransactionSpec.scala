...
class TransactionSpec extends FunSuite with Matchers {
  test("create and sign transaction") {
    ...
    val pkey = PrivateKey.fromBase58("92f9274aR3s6zd1vuAgxquv4KP5S5thJadF3k54NHuTV4gn4CTc", Base58.Prefix.SecretKeyTestnet)._1
    ...
    assert(res === hex"")
  }
  ...
  test("create and verify pay2pk transactions with 1)put/1 output") {
    ...
    val providedHash = Base58Check.decode("mhW1BQDyhbTsnHEuB1n7yuj9V81TbuKWRh")._2
  ...
  test("create and verify sign pay2pk transactions with multiple)puts and outputs") {
    ...
    val keys = List(
      PrivateKey.fromBase58("cV7LGVeY2VPuCyCSarqEqFCUNig2NzwiAEBTTA89vNRQ4VnVN7So", Base58.Prefix.SecretKeyTestnet)._1,
      PrivateKey.fromBase58("93NJN4mhL21FxRbfHZJ2Cou1YnrJmWNkujmZxeT7CPKauEZJnmi", Base58.Prefix.SecretKeyTestnet)._1
    )
  ...
  test("create and sign p2sh transactions") {
    ...
    val privateKey = PrivateKey.fromBase58("92TgRLMLLdwJjT1JrrmTTWEpZ8uG7zpHEgSVPTbwfAs27Pea1UQ", Base58.Prefix.SecretKeyTestnet)._1
    val publicKey = privateKey.publicKey
  ...
  test("sign a 3-to-2 transaction with helper method") {
    ...
    val keys = List(
      PrivateKey.fromBase58("cW6bSKtH3oMPA18cXSMR8ASHztrmbwmCyqvvN8x3Tc7WG6TMdLme", Base58.Prefix.SecretKeyTestnet)._1,
      PrivateKey.fromBase58("93Ag8t83NW9WmPbhqLCSUNckARpbpgWtp4EWGidtj6h6pWwDZZU", Base58.Prefix.SecretKeyTestnet)._1,
      PrivateKey.fromBase58("921vnTeSQCN7GMHdiHyaoZ1JSugTtzvg8rqyXH9HmFtBgoPHPKs", Base58.Prefix.SecretKeyTestnet)._1
    )
